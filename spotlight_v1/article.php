...
<body>
    ...
    <nav class="navbar navbar-expand-lg ">
      ...
      if(isset($_GET['title']) && isset($_GET['date'])){
        ...
        $sqlMusic = "SELECT * FROM music WHERE music_title='$title' AND music_date='$date'";
        ...
        if($queryResultsUsers > 0){
            ...
            if($queryResultsMusic > 0){
              ...
              while($rowMusic = mysqli_fetch_assoc($resultMusic)){
                  ...
                  <div class='main-content'>
                  <div class='article-container'>
                  <img src='{$rowMusic['music_image']}' alt=''>
                  <h3>{$rowMusic['music_date']} | MUSIK | {$rowMusic['music_author']}</h3>
                  <h2>{$rowMusic['music_title']}</h2>
                  <p style='white-space: pre-wrap;'>{$rowMusic['music_content_article']}</p>      
                  </div>";
      ...
      <h1 class="comment-title-section">KOMMENTARSFÄLT</h1>
        ...
        if($numComments > 0){
          while($rowComment = mysqli_fetch_assoc($resultComment)){
            ...
            $rowCommentFiltered = htmlspecialchars($rowComment['comment']);
           ...
           echo "<div class='comment'>
            ...
            echo "
              <div class='userComment' style='white-space: pre-wrap;'>{$rowCommentFiltered}</div>
            </div>";
          }
        }
        else{
